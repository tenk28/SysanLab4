import random
from solutor import *
from cus_class import SolveExpTh
from df_read import read_data
from view import OperatorViewWindow
...
START_FUEL = 47
TIME_DELTA = 10
reason = [u'Low board voltage\n',u'Lack of fuel\n',u'Low AB battery charge\n']
...
def classify_danger_rating(level):
    if 0 <= level <= 0.07:
        return 0, u"Safe situation"
    elif 0.07 < level <= 0.25:
        return 1, u"Abnormal situation with just single parameter"
    elif 0.25 < level <= 0.375:
        return 2, u"Abnormal situation with several parameters"
    elif 0.375 < level <= 0.5:
        return 3, u"Accident casing can be noticed"
    elif 0.5 < level <= 0.625:
        return 4, u"High accident likelihood"
    elif 0.625 < level <= 0.75:
        return 5, u"Critical situation"
    elif 0.75 < level <= 0.875:
        return 6, u"Low chance to avoid"
    elif 0.875 < level <= 1:
        return 7, u"Accident!"


class SolverManager(object):
    Y_C = np.array([[11.7], [1], [12]])
    Y_D = np.array([[10.5], [0.0], [10.5]])
    ...
    def __init__(self, d):
        ...
        self.operator_view = OperatorViewWindow(warn=self.Y_C, fail=self.Y_D, callback=self,
                                                descriptions=[u'Напруга', u'Запас\ палива',
                                                              u'Напруга\ в\ АБ'],
    ...
    def launch(self):
        if self.current_iter + self.batch_size  < len(self.data):
            ...
            self.current_iter += self.forecast_size
    ...
    def predict(self, shift, n):
        ...
        voltage = np.array(list(
            [x[0] for x in self.solver.X_[0][-self.forecast_size:, 0].tolist()]
        ))
    ...
    def rdr_calc(self):
        ...
        for i in range(3):
            ...
            if t < rdr:
                rdr = t
        if rdr != np.inf:
            self.rdr = rdr * (self.batch_size + self.forecast_size)
